...
//		datas.forEach(System.out::println);// 참조형
...
//      map() : 기존값을 원하는 값으로 변경
		String data1 = "ABC";
		data1.chars().map(number -> number + 3).forEach(number -> System.out.print((char) number));
...
//		문자열 타입이 아닌 값은 joining을 사용할 수 없다.
		String results = datas.stream().sorted().map(String::valueOf).collect(Collectors.joining(" "));
		System.out.println(results);
