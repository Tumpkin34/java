...
//	추가와 삽입
	public boolean add(BoxOfficeVO boxOfficeVO) throws IOException{//이메소드를 쓰는 곳에서 예외처리 떠넘김
//		받아온 영화 정보의 랭킹이 없으면 마지막에 추가 있으면 해당 랭킹에 삽입 
...
//	마지막에 추가하는 정보이므로 순위는 전달받지 않는다.
	private boolean append(BoxOfficeVO boxOfficeVO) throws IOException{
		BufferedWriter bufferedWriter = DBConnecter.getAppend();//추가로 연결하는 버퍼
		BufferedReader bufferedReader = DBConnecter.getReader();//읽는 버퍼
		String content = null, newLine = null, line = null;
		int lastRanking = 0;
		boolean check = true;
		
		while((line = bufferedReader.readLine()) != null) {//한줄씩 가져옴
//			기존 정보 중 순위를 제외한 5개 정보
			String excludedRankingOrigin = line.substring(line.indexOf("\t"));//처음만난\t를 기준으로 뒷부분 저장(랭킹제외한 정보)
//			전달받은 정보 중 순위를 제외한 5개 정보
			String excludedRankingNew = boxOfficeVO.toString();//받아온 객체 toString으로 재정의된 문자열로 저장
			//받아온 객체도 \t를 기준으로 나눈 뒷부분(랭킹제외한정보) 저장
...
//	조회
	public ArrayList<BoxOfficeVO> select(String name) throws IOException{
		...
		while((line = bufferedReader.readLine()) != null) {
			String[] datas = line.split("\t");//뽑아온 내용 배열에 담음
			if(datas[1].contains(name)) {//1번 인덱스번호의 이름에 받아온 문자열이 포함되었다면
				BoxOfficeVO boxOfficeVO = new BoxOfficeVO();//객체만들고
//				배열의 정보들로 초기화
				boxOfficeVO.setRanking(Integer.valueOf(datas[i++]));
				boxOfficeVO.setName(datas[i++]);
				boxOfficeVO.setReleaseDate(datas[i++]);
//				매출에 대한 정보가 없다면("") 0을 아니면 매출액으로 초기화
...
//				조회된 내용들 저장
				...
				i = 0;//위에서 인덱스번호로 사용했던것이니까 반복돌려면 다시 0으로 초기화
			}
		}
		return boxOfficeVOs;//찾은 영화들 주소 넘김
	}
	
//	목록
//	바로위의 조회의 메소드와 같지만 if문만 없앤 것
	...
	public String removeComma(String data) {//,없애주는 메소드
		return data.replaceAll(",", "");
	}
	
	public String removeS(String data) {//S없애주는 메소드
