...
//			}
//		}
//		위의 for문을 밑의 빠른 for문으로 변경
		for (User temp : users) {//빠른 for문
...
//	- 암호화
	public String encrypt(String password) {
		String decryptedPassword = "";//암호화한 pw저장 변수

		for (int i = 0; i < password.length(); i++) {
			char c = password.charAt(i);//받아온 값의 i번째 인덱스 저장
			decryptedPassword += (char) (c * KEY);//저장된 문자를 KEY와 연산해 다시 문자로 형변환후 연결하여 저장
		}

		return decryptedPassword;//암호화된 비밀번호 리턴
	}

//	- 비밀번호 변경(비밀번호 찾기 서비스)
	public void changePassword(User user) {//변경한 비밀번호가 담긴 주소 받음
