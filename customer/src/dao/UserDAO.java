...
//	   받아온 아이디가 userId랑 같다면 해당 로우의 userStatus 조회 하는 쿼리문 작성
...
//      로그인 
		...
		try {
			...
			if (resultSet.next()) {//next해서 true라는 것은 중복된 아이디가 있다는 뜻
				code = DUPLICATED_ID_CODE;//중복된 아이디가 있다는 뜻의 정수 저장

				if (resultSet.getInt(1) == DELETED_USER_CODE) {//근데 그 코드가 사실 탈퇴한 아이디라면
					code = DELETED_USER_CODE;//탈퇴한 아이디라는 뜻의 정수 저장

				}
			} else {
				code = ENABLED_ID_CODE;//중복된 아이디가 없다는 뜻의 정수 저장
			}
		...
		return code;//위에서 각 조건에 따라 저장된 정수 리턴
	}
